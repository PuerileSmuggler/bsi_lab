export const UserActionTypes = {
  Login: "USER_LOGIN",
  LoginSuccess: "USER_LOGIN_SUCCESS",
  LoginError: "USER_LOGIN_ERROR",
  RefreshToken: "USER_REFRESH_TOKEN",
  RefreshTokenSuccess: "USER_REFRESH_TOKEN_SUCCESS",
  RefreshTokenError: "USER_REFRESH_TOKEN_ERROR",
  ClearLoginError: "USER_CLEAR_LOGIN_ERROR",
  Register: "USER_REGISTER",
  RegisterSuccess: "USER_REGISTER_SUCCESS",
  RegisterError: "USER_REGISTER_ERROR",
  Logout: "USER_LOGOUT",
  LogoutSuccess: "USER_LOGOUT_SUCCESS",
  LogoutError: "USER_LOGOUT_ERROR",
  CreatePassword: "USER_CREATE_PASSWORD",
  CreatePasswordSuccess: "USER_CREATE_PASSWORD_SUCCESS",
  CreatePasswordError: "USER_CREATE_PASSWORD_ERROR",
  GetAllPasswords: "USER_GET_ALL_PASSWORDS",
  GetAllPasswordsSuccess: "USER_GET_ALL_PASSWORDS_SUCCESS",
  GetAllPasswordsError: "USER_GET_ALL_PASSWORDS_ERROR",
  GetAllSharedPasswords: "USER_GET_ALL_SHARED_PASSWORDS",
  GetAllSharedPasswordsSuccess: "USER_GET_ALL_SHARED_PASSWORDS_SUCCESS",
  GetAllSharedPasswordsError: "USER_GET_ALL_SHARED_PASSWORDS_ERROR",
  DeletePassword: "USER_DELETE_PASSWORD",
  DeletePasswordSuccess: "USER_DELETE_PASSWORD_SUCCESS",
  DeletePasswordError: "USER_DELETE_PASSWORD_ERROR",
  EditPassword: "USER_EDIT_PASSWORD",
  EditPasswordSuccess: "USER_EDIT_PASSWORD_SUCCESS",
  EditPasswordError: "USER_EDIT_PASSWORD_ERROR",
  EditUser: "USER_EDIT_USER",
  EditUserSuccess: "USER_EDIT_USER_SUCCESS",
  EditUserError: "USER_EDIT_USER_ERROR",
  GetPasswordById: "USER_GET_PASSWORD_BY_ID",
  GetPasswordByIdSuccess: "USER_GET_PASSWORD_BY_ID_SUCCESS",
  GetPasswordByIdError: "USER_GET_PASSWORD_BY_ID_ERROR",
  ClearPassword: "USER_CLEAR_PASSWORD",
  ClearIpBlock: "USER_CLEAR_IP_BLOCK",
  ClearIpBlockError: "USER_CLEAR_IP_BLOCK_ERROR",
  ClearIpBlockSuccess: "USER_CLEAR_IP_BLOCK_SUCCESS",
  SetIpBlock: "USER_SET_IP_BLOCK",
  sharePassword: "USER_SHARE_PASSWORD",
  sharePasswordSuccess: "USER_SHARE_PASSWORD_SUCCESS",
  sharePasswordError: "USER_SHARE_PASSWORD_ERROR",
  sharingPasswords: "USER_SHARING_PASSWORDS",
  sharingPasswordsSuccess: "USER_SHARING_PASSWORDS_SUCCESS",
  sharingPasswordsError: "USER_SHARING_PASSWORDS_ERROR",
  removeSharingPasswords: "USER_REMOVE_SHARING_PASSWORDS",
  removeSharingPasswordsSuccess: "USER_REMOVE_SHARING_PASSWORDS_SUCCESS",
  removeSharingPasswordsError: "USER_REMOVE_SHARING_PASSWORDS_ERROR",
};
